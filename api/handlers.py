...
from api.models import UpdateUserRequest, UserCreate, ShowUser, DeleteUserResponse, UpdateUserResponse
...
async def update_user_byt_id(user_id: UUID,
                             body: UpdateUserRequest,
                             db: AsyncSession=Depends(get_db)) -> UpdateUserResponse:
    updated_user_params = body.dict(exclude_unset=True)
    if updated_user_params == {}:
        raise HTTPException(status_code=422, detail="At least one field should be updated")
    user = await _get_user_by_id(user_id, db)
    if user is None:
        raise HTTPException(status_code=404, detail="User with id {user_id} not found")
